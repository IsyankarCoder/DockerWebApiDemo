...
WORKDIR /src
COPY ["DockerWebApiDemo.csproj", "."]
RUN dotnet restore "./DockerWebApiDemo.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./DockerWebApiDemo.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./DockerWebApiDemo.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "DockerWebApiDemo.dll"]